...
def _fetch_jin10_quote_p0(code: str, field: str) -> tuple[float | None, str | None, list[str]]:
    """Fetch realtime/open P0 via configured Jin10 MCP."""
    mcp_python = Path(os.getenv("JIN10_MCP_PYTHON", sys.executable)).expanduser()
    if not mcp_python.exists():
        return None, None, ["jin10_mcp_python_missing"]
    mcp_config = Path(
        os.getenv("JIN10_MCP_CONFIG", str(Path.home() / ".finance-agent" / "mcp.yaml"))
    ).expanduser()
    if not mcp_config.exists():
        return None, None, ["jin10_mcp_config_missing"]
    probe_path = Path("/tmp/finance_agent_jin10_quote_probe.py")
    probe_path.write_text("import asyncio, json, sys, yaml\nfrom datetime import timedelta\nfrom pathlib import Path\nfrom mcp.client.streamable_http import streamablehttp_client\nfrom mcp import ClientSession\ncode=sys.argv[1]\nfield=sys.argv[2]\nconfig_path=Path(sys.argv[3]).expanduser()\ncfg=yaml.safe_load(config_path.read_text())\nserver=(cfg.get('mcp_servers') or {}).get('jin10')\nif not server:\n    print(json.dumps({'error':'jin10_mcp_not_configured'})); raise SystemExit(0)\nasync def main():\n    async with streamablehttp_client(server['url'], headers=server.get('headers') or {}, timeout=timedelta(seconds=60)) as (read, write, _):\n        async with ClientSession(read, write, read_timeout_seconds=timedelta(seconds=60)) as session:\n            await session.initialize()\n            res=await session.call_tool('get_quote', {'code': code})\n            payload=res.model_dump(mode='json', exclude_none=True) if hasattr(res,'model_dump') else {}\n            data=(payload.get('structuredContent') or {}).get('data') or {}\n            print(json.dumps({'value':data.get(field),'time':data.get('time'),'code':data.get('code'),'name':data.get('name'),'field':field}, ensure_ascii=False))\nasyncio.run(main())\n", encoding="utf-8")
    try:
        proc = subprocess.run(
            [str(mcp_python), str(probe_path), code, field, str(mcp_config)],
            check=False,
            text=True,
            capture_output=True,
            timeout=90,
