...
Environment overrides:
  ...
  JIN10_BROWSER_PROFILE=/path/to/browser-profile
EOF
}
...
case "$MODE" in
  stdio-local|http-local)
    PROFILE="${PLAYWRIGHT_MCP_PROFILE:-$HOME/.finance-agent/playwright_mcp_profile/finance-agent}"
    OUTPUT_DIR="$OUTPUT_ROOT/local"
    ;;
  stdio-jin10|http-jin10)
    PROFILE="${PLAYWRIGHT_MCP_PROFILE:-${JIN10_BROWSER_PROFILE:-}}"
    if [[ -z "$PROFILE" ]]; then
      echo "ERROR: set JIN10_BROWSER_PROFILE or PLAYWRIGHT_MCP_PROFILE for Jin10 mode" >&2
      exit 2
    fi
