...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build one authoritative XAUUSD Gold Policy daily report package."
    )
    ...
    parser.add_argument("--snapshot-path", default=None, help="Optional exact premarket_snapshot.json path under storage root.")
    parser.add_argument("--run-id", default=None, help="Optional immutable report run id.")
    parser.add_argument("--dry-run", action="store_true", help="Validate inputs and print the planned immutable bundle path without writing.")
    return parser
...
def run_gold_daily_report(
    *,
    ...
    """Execute the typed Gold Policy close and return only formal package paths."""
    ...
    try:
        ...
        controls = build_gold_daily_close_runtime_controls(
            ...
            decision_as_of=_decision_as_of(snapshot, current),
        )
...
def _existing_completed_result(
    *,
    ...
    result = _read_json(bundle_path / "gold_daily_close_loop_result.v1.json")
    data_quality = _read_json(bundle_path / "data_quality.json")
    report_manifest = _read_json(bundle_path / "report_manifest.json")
    if not report_paths or not result or not data_quality or not report_manifest:
        return None
    return {
        "status": "completed",
        "trade_date": trade_date,
        ...
        "daily_close_result_id": result.get("result_id"),
