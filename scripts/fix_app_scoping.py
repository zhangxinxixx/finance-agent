"""Step 1: 删除误写到 user_id=xinxi 的 app 级记忆。
Step 2: 用原生 app_id/agent_id 重新写入。
"""
...
# ── Step 1: 找出并删除 source=app_fas_recovery_2026-05-16 的条目 ──
print("=== Step 1: 清理误写条目 ===")
r = c.get_all(filters={"user_id": "xinxi"}, top_k=100)
to_delete = [i["id"] for i in r.get("results", [])
             if i["metadata"].get("source") == "app_fas_recovery_2026-05-16"]

print(f"找到 {len(to_delete)} 条待删除")
ok = fail = 0
for mid in to_delete:
    try:
        c.delete(memory_id=mid)
        ok += 1
    except Exception as e:
        print(f"  FAIL: {str(e)[:80]}")
        fail += 1
print(f"删除: {ok}/{len(to_delete)} (失败 {fail})")

# ── Step 2: 用原生 app_id 重写 ──
print("\n=== Step 2: 原生 app_id 写入 ===")
...
memories = [
    ...
    ("current_priority", "优先级：① Dashboard v0.3 Phase 1 布局重构（3列+Header+右侧面板+导航）。② Options wall 多日校准收尾。③ 市场赔率/事件概率层完善。④ Hermes/Codex Mem0 执行流程固化。", ["priority", "dashboard", "options"]),
]

for i, (mt, content, tags) in enumerate(memories):
    r = c.add(
        messages=[{"role": "user", "content": content}],
        app_id=APP_ID,  # ← 原生 app_id，不传 user_id
        metadata={
            "memory_type": mt,
            "importance": "high",
            "tags": tags + [APP_ID, "app"],
            "source": "native_app_id_2026-05-16",
        },
        infer=False,
    )
...
print(f"\nDone: {len(memories)} entries written to app_id={APP_ID}")
