...
PROJECT_DOCS_PRESET: tuple[PublishDocumentSpec, ...] = (
    ...
    PublishDocumentSpec(
        title="finance-agent 页面规格",
        markdown_paths=(
            Path("docs/frontend/page-specs/dashboard.md"),
            Path("docs/frontend/page-specs/data-ingestion.md"),
            Path("docs/frontend/page-specs/market-monitor.md"),
            Path("docs/frontend/page-specs/cme-options.md"),
        ),
    ),
)
...
def _tenant_doc_base_url(openapi_base_url: str) -> str:
    explicit_base = os.getenv("FEISHU_WEB_BASE_URL") or os.getenv("LARK_WEB_BASE_URL")
    if explicit_base:
        return explicit_base.rstrip("/")
    if "larksuite.com" in openapi_base_url:
        return "https://larksuite.com"
    return "https://feishu.example.com"
