...
DEFAULT_EXTERNAL_ROOT = Path(os.getenv("JIN10_REPORTS_ROOT", "~/jin10-reports")).expanduser()
...
DEFAULT_BROWSER_PROFILE = Path(os.getenv("JIN10_BROWSER_PROFILE")).expanduser() if os.getenv("JIN10_BROWSER_PROFILE") else None
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--browser-profile",
        default=str(DEFAULT_BROWSER_PROFILE) if DEFAULT_BROWSER_PROFILE and DEFAULT_BROWSER_PROFILE.exists() else None,
    )
    parser.add_argument("--model", default=DEFAULT_MODEL, help="LLM model for agent analysis.")
    parser.add_argument("--qwen-model", default=DEFAULT_QWEN_MODEL, help="Vision model for page parsing.")
...
def main() -> int:
    ...
    env["JIN10_QWEN_VL_MODEL"] = args.qwen_model
