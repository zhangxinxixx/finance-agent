...
def test_higher_timeframe_uses_one_complete_provider_bucket() -> None:
    base = datetime(2026, 7, 16, 9, 0, tzinfo=UTC)
    twelve = [
        _row(base + timedelta(minutes=5 * index), source="twelvedata_xauusd_5m", price=3900 + index)
        for index in range(3)
    ]
    incomplete_jin10 = [
        _row(base, source="jin10_mcp_derived_5m", price=4100),
        _row(base + timedelta(minutes=10), source="jin10_mcp_derived_5m", price=4200),
    ]

    result = aggregate_provider_complete_candles(
        [*twelve, *incomplete_jin10],
        source_timeframe="5m",
        target_timeframe="15m",
        source="canonical_xauusd_5m_aggregate_15m",
    )

    assert len(result) == 1
    assert result[0].open == 3900
    assert result[0].close == 3902.5
    assert result[0].source_ref["selected_provider"] == "twelvedata_xauusd_5m"
    assert {item["source"] for item in result[0].source_ref["component_source_refs"]} == {
