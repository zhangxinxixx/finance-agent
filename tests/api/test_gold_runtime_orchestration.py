...
def test_gold_runtime_orchestration_contract_declares_required_run_modes() -> None:
    ...
    assert "source_health_agent" in premarket["agents_executed"]
    assert "gold_macro_overview_agent" in premarket["agents_executed"]
    assert "review_gate_agent" in premarket["agents_executed"]
    assert "report_render_agent" in premarket["agents_executed"]
...
def test_intraday_mode_is_incremental_and_mixed_capable() -> None:
    ...
    assert "driver_decomposition_agent" in summary["agents_executed"]
...
def test_major_event_reprice_routes_geopolitical_oil_events_to_war_oil_rate_chain() -> None:
    ...
    assert "review_gate_agent" in summary["agents_executed"]
...
def test_postmarket_mode_runs_report_and_system_evolution() -> None:
    ...
    assert "system_evolution_agent" in summary["agents_executed"]
...
def test_version_change_validation_runs_schema_dag_and_test_validation_agents() -> None:
    ...
    assert "schema_agent" in summary["agents_executed"]
    assert "dag_lineage_agent" in summary["agents_executed"]
    assert "test_validation_agent" in summary["agents_executed"]
