...
def test_v2_five_day_runtime_replay_reads_only_the_previous_v2_head(
    ...
    forbidden_artifacts = {
        name.replace(".v2.json", ".v1.json") for name in expected_artifacts if name.endswith(".v2.json")
    }
...
def test_v2_prebootstrap_blocked_run_persists_without_v1_delivery_artifacts(
    ...
    assert "gold_policy_report_render.v2.json" in names
    assert "gold_policy_report_render.v1.json" not in names
    assert {
        "source.md",
        "analysis.md",
        "visual.html",
