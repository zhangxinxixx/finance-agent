...
def test_runtime_commits_linked_same_session_revision_and_selects_latest(
    ...
    assert second.predecessor_lookup.status == "found"
    assert second.predecessor_lookup.latest_receipt is not None
    assert second.predecessor_lookup.latest_receipt.receipt_id == first.write_result.receipt_id
...
def test_runtime_existing_bundle_excludes_its_own_session_predecessor(
    tmp_path: Path,
) -> None:
    bootstrap_input, _ = _bootstrap_pair()
    first = execute_gold_daily_close_runtime(
        storage_root=tmp_path,
        run_id="runtime-idempotent",
        current_feature=bootstrap_input.current_feature,
        controls=_controls(bootstrap_input),
        bootstrap_previous_feature=bootstrap_input.previous_feature,
    )
    ...
    assert replay.predecessor_lookup.status == "missing"
    assert replay.loop_input == first.loop_input
    assert replay.write_result.receipt_id == first.write_result.receipt_id
