...
export const CATEGORY_MAP: Record<string, { label: string; color: string }> = {
  ...
  macro_report: { label: "宏观分析报告", color: "#38bdf8" },
...
export const TYPE_DESCRIPTIONS: Record<string, { summary: string; tags: string[] }> = {
  ...
  macro_report: { summary: "宏观分析报告聚焦流动性、实际利率、美元和黄金阶段判断。", tags: ["宏观", "分析"] },
  weekly_context_revision: { summary: "周报修订以金十周报为不可变锚点，按最新价格、利率、期权、持仓和新闻证据逐项微调结论。", tags: ["周报", "上下文修订"] },
...
export function getReportTitle(item: ReportIndexItem): string {
  ...
  if (item.type === "macro_report") return `XAUUSD 宏观分析报告 · ${dateLabel}`;
