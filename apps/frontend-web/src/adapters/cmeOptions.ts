...
import type {
  ...
  CMEOptionsDecisionLargeOiLevel,
  CMEOptionsDecisionMetric,
  CMEOptionsDecisionOiChange,
...
function normalizeDecisionKeyLevels(value: unknown): CMEOptionsDecisionKeyLevel[] {
  ...
  return value.map((item) => {
    ...
    return {
      ...
      structural_role_at_report: asString(raw.structural_role_at_report, asString(raw.role, "unavailable")),
      current_relation: raw.current_relation === "below_price" || raw.current_relation === "above_price" || raw.current_relation === "at_price"
        ? raw.current_relation
        : null,
      dynamic_role: asString(raw.dynamic_role, asString(raw.role, "unavailable")),
    };
  });
}

function normalizeLargeOiLevels(value: unknown): CMEOptionsDecisionLargeOiLevel[] {
  if (!Array.isArray(value)) return [];
  return value.map((item) => {
    const raw = item && typeof item === "object" ? item as Record<string, unknown> : {};
    return {
      ...
      strike: asNullableNumber(raw.strike),
      call_oi: asNullableNumber(raw.call_oi),
      put_oi: asNullableNumber(raw.put_oi),
      total_oi: asNullableNumber(raw.total_oi),
      total_oi_change: asNullableNumber(raw.total_oi_change),
      volume: asNullableNumber(raw.volume),
      distance_pct: asNullableNumber(raw.distance_pct),
      dominant_side: asString(raw.dominant_side, "BALANCED"),
    };
  });
}

function normalizeOiChanges(value: unknown): CMEOptionsDecisionOiChange[] {
  if (!Array.isArray(value)) return [];
  return value.map((item) => {
    const raw = item && typeof item === "object" ? item as Record<string, unknown> : {};
    return {
      ...
      current_oi: asNullableNumber(raw.current_oi),
      previous_oi: asNullableNumber(raw.previous_oi),
      delta: asNullableNumber(raw.delta),
      volume: asNullableNumber(raw.volume),
      block: asNullableNumber(raw.block),
      pnt: asNullableNumber(raw.pnt),
    };
  });
}
...
function normalizeDecisionResponse(payload: unknown): CMEOptionsDecisionResponse {
  ...
  const rankings = raw.oi_change_rankings && typeof raw.oi_change_rankings === "object" ? raw.oi_change_rankings as Record<string, unknown> : {};
  const pnt = raw.pnt_summary && typeof raw.pnt_summary === "object" ? raw.pnt_summary as Record<string, unknown> : {};
  const pntTotals = pnt.totals && typeof pnt.totals === "object" ? pnt.totals as Record<string, unknown> : {};
  const pntOnlyTotals = pnt.pnt_totals && typeof pnt.pnt_totals === "object" ? pnt.pnt_totals as Record<string, unknown> : pntTotals;
  const blockTotals = pnt.block_totals && typeof pnt.block_totals === "object" ? pnt.block_totals as Record<string, unknown> : {};
  const intent = raw.intent_summary && typeof raw.intent_summary === "object" ? raw.intent_summary as Record<string, unknown> : {};
  const structure = raw.structure_summary && typeof raw.structure_summary === "object" ? raw.structure_summary as Record<string, unknown> : {};
  ...
  return {
    ...
    price_context: {
      report_p0: asNullableNumber(prices.report_p0),
      report_p0_source: typeof prices.report_p0_source === "string" ? prices.report_p0_source : null,
      report_p0_timestamp: typeof prices.report_p0_timestamp === "string" ? prices.report_p0_timestamp : null,
      live_p0: asNullableNumber(prices.live_p0),
      live_p0_source: typeof prices.live_p0_source === "string" ? prices.live_p0_source : null,
      live_p0_timestamp: typeof prices.live_p0_timestamp === "string" ? prices.live_p0_timestamp : null,
      live_price_status: prices.live_price_status === "fresh" || prices.live_price_status === "stale" || prices.live_price_status === "future"
        ? prices.live_price_status
        : "unavailable",
      live_price_freshness_seconds: asNullableNumber(prices.live_price_freshness_seconds),
      live_price_coverage_status: prices.live_price_coverage_status === "complete" || prices.live_price_coverage_status === "degraded"
        ? prices.live_price_coverage_status
        : "unavailable",
      model_f: asNumberRecord(prices.model_f),
      price_anchor_rule: typeof prices.price_anchor_rule === "string" ? prices.price_anchor_rule : null,
    },
    ...
    large_oi_levels: normalizeLargeOiLevels(raw.large_oi_levels),
    nearby_large_oi_levels: normalizeLargeOiLevels(raw.nearby_large_oi_levels),
    oi_change_rankings: {
      comparison_status: rankings.comparison_status === "available" ? "available" : "unavailable",
      largest_increases: normalizeOiChanges(rankings.largest_increases),
      largest_decreases: normalizeOiChanges(rankings.largest_decreases),
    },
    pnt_summary: {
      status: asDecisionStatus(pnt.status),
      totals: { call: asNullableNumber(pntTotals.call), put: asNullableNumber(pntTotals.put), total: asNullableNumber(pntTotals.total) },
      pnt_totals: { call: asNullableNumber(pntOnlyTotals.call), put: asNullableNumber(pntOnlyTotals.put), total: asNullableNumber(pntOnlyTotals.total) },
      block_totals: { call: asNullableNumber(blockTotals.call), put: asNullableNumber(blockTotals.put), total: asNullableNumber(blockTotals.total) },
      block_coverage_status: pnt.block_coverage_status === "observed" || pnt.block_coverage_status === "not_verified" ? pnt.block_coverage_status : "unavailable",
      warnings: asStringArray(pnt.warnings),
      top_activity: Array.isArray(pnt.top_activity) ? pnt.top_activity.filter((item): item is Record<string, unknown> => Boolean(item) && typeof item === "object") : [],
    },
    ...
    intent_summary: {
      type: typeof intent.type === "string" ? intent.type : null,
      score: asNullableNumber(intent.score),
      confidence: asNullableNumber(intent.confidence),
      wording: typeof intent.wording === "string" ? intent.wording : null,
      scores: Object.fromEntries(Object.entries(intent.scores && typeof intent.scores === "object" ? intent.scores as Record<string, unknown> : {}).map(([key, value]) => [key, asNullableNumber(value)])),
      evidence: asStringArray(intent.evidence),
    },
    structure_summary: {
      state: asString(structure.state, "unavailable"),
      label: asString(structure.label, "结构不可用"),
      summary: asString(structure.summary, "后端未返回结构摘要。"),
      reference_price: asNullableNumber(structure.reference_price),
      gamma_zero: asNullableNumber(structure.gamma_zero),
      below_gamma_zero: asBoolean(structure.below_gamma_zero),
      net_gex: asNullableNumber(structure.net_gex),
      net_gex_change: asNullableNumber(structure.net_gex_change),
    ...
    scenario_paths: Array.isArray(raw.scenario_paths) ? raw.scenario_paths.map((item) => {
      const path = item && typeof item === "object" ? item as Record<string, unknown> : {};
      return { path_id: asString(path.path_id), label: asString(path.label), status: asString(path.status), triggers: asStringArray(path.triggers), targets: asNumberArray(path.targets), invalidation: asStringArray(path.invalidation) };
    }) : [],
