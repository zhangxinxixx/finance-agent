...
export function useLLMAudits(filters: LLMAuditFilters, auditToken = "") {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setError(null);
    void fetchLLMAudits(filters, auditToken)
      .then((next) => {
  ...
  }, [auditToken, filters.caller, filters.model, filters.provider, filters.reportId, filters.runId, filters.status, filters.tradeDate, reloadToken]);
