...
export function LLMAuditPage() {
  ...
  const audits = useLLMAudits(filters);
  ...
  useEffect(() => {
    ...
    void fetchLLMAuditDetail(selectedId).then((item) => { if (!cancelled) setSelected(item); }).catch(() => { if (!cancelled) setSelected(null); });
    return () => { cancelled = true; };
  }, [selectedId]);

  if (audits.isLoading && !audits.data) return <div className="finance-page-shell"><LoadingSkeleton variant="page" /></div>;
  if (audits.error) return <div className="finance-page-shell"><div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-card)] p-6 text-[12px] text-[var(--status-down)]">{audits.error.message}</div></div>;
    ...
    <div className="finance-page-shell">
      <div className="space-y-4">
        ...
        <div className="grid min-h-0 gap-4 xl:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.7fr)]">
          ...
          <LLMAuditDetailPanel audit={selected} />
        </div>
      </div>
    </div>
  );
