...
def api_tasks(limit: int = 20):
    ...
    return {"tasks": api_main.list_recent_tasks(min(limit, 100))}
...
def api_scheduler_overview(days: int = 7, limit: int = 50, db: Session = Depends(get_db)):
    ...
    return api_main.get_scheduler_overview(db, days=min(days, 90), limit=min(limit, 200))
...
def api_dashboard_summary():
    ...
    return api_main.get_dashboard_summary()
