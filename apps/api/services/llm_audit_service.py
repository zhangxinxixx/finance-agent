...
def get_llm_audit_view(db, audit_id: str, *, include_content: bool = False) -> dict[str, Any] | None:
    row = get_llm_call_audit(db, audit_id)
    return audit_detail(row, include_content=include_content) if row else None
...
def audit_detail(row: LLMCallAudit, *, include_content: bool = True) -> dict[str, Any]:
    payload = audit_summary(row)
    request_config = row.request_config or {}
    payload.update(
        {
            "request_messages": (row.request_messages or []) if include_content else [],
            "response_text": row.response_text if include_content else None,
            "attempts": (row.attempts or []) if include_content else [],
            "context": (row.context or {}) if include_content else {},
            "source_refs": (row.source_refs or []) if include_content else [],
            "secrets_redacted": bool(request_config.get("secrets_redacted")),
            "sanitization_performed": bool(request_config.get("sanitization_performed")),
...
def build_report_llm_audit_view(row: AgentOutput) -> dict[str, Any]:
    ...
    return {
        ...
        "note": (
            "完整 Prompt/输入/输出仅通过受权限保护的 LLM Audit 详情接口读取。"
            if has_trace
            else "历史 AgentOutput 未记录完整实际 LLM 输入输出，未回填或推测。"
        ),
