...
def get_orchestration_latest(*, storage_root: Path | str = _STORAGE_ROOT, date: str | None = None) -> dict[str, Any]:
    ...
    summary = _read_json(_artifact_path(root, trade_date, "automation_summary.json"))
    workflow_runs = _read_json(_artifact_path(root, trade_date, "workflow_runs.json"))
    ...
    pre_analysis_gate = _read_json(_artifact_path(root, trade_date, "pre_analysis_gate.json"))
    retry_queue = _read_json(_artifact_path(root, trade_date, "retry_queue.json"))
...
def get_orchestration_notification_plan(*, storage_root: Path | str = _STORAGE_ROOT, date: str | None = None) -> dict[str, Any]:
    root = Path(storage_root)
    trade_date = date or _latest_date(root)
    payload = _read_json(_artifact_path(root, trade_date, "notification_plan.json"))
    if payload:
        return payload
