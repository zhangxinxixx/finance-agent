...
def _summarize(values: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    lifecycle_counts = Counter(_lifecycle_status(item) for item in values)
    ...
    unknown_lifecycles = set(lifecycle_counts) - set(_LIFECYCLE_STATUSES)
    if unknown_lifecycles:
        raise ValueError(f"unsupported outcome lifecycle_status: {sorted(unknown_lifecycles)!r}")

    verified_scored = [
        item
        for item in values
        if item.get("status") == "scored" and _lifecycle_status(item) in _TRUSTED_SCORED_LIFECYCLES
    ]
    directional = [item for item in verified_scored if item.get("direction_accuracy") in {"correct", "incorrect"}]
    mfe_values = [_number(item.get("mfe")) for item in verified_scored]
    mae_values = [_number(item.get("mae")) for item in verified_scored]
    ...
    return {
        ...
        "verified_scored_count": len(verified_scored),
        "legacy_unverified_count": lifecycle_counts.get("legacy_unverified", 0),
        ...
        "lifecycle_counts": {key: lifecycle_counts.get(key, 0) for key in _LIFECYCLE_STATUSES},
    }


def _lifecycle_status(payload: Mapping[str, Any]) -> str:
    value = payload.get("lifecycle_status")
