...
_STAGE_GRAPH: dict[AnalysisStage, frozenset[AnalysisStage]] = {
    AnalysisStage.PRESSURE: frozenset(
        {AnalysisStage.RANGE, AnalysisStage.WEAK_REPAIR}
    ),
    AnalysisStage.RANGE: frozenset(
        {AnalysisStage.PRESSURE, AnalysisStage.DIRECTION_DECISION}
    ),
    AnalysisStage.DIRECTION_DECISION: frozenset(
        {AnalysisStage.RANGE, AnalysisStage.WEAK_REPAIR}
    ),
    ...
    AnalysisStage.REVERSAL_WATCH: frozenset(
        {AnalysisStage.WEAK_REPAIR, AnalysisStage.TREND_CONFIRMED}
    ),
...
def evaluate_analysis_state_transition(
    ...
    *,
    ...
    if analysis.quality_status != "accepted":
        reason = (
            "ANALYSIS_DECISION_BLOCKED"
            if analysis.quality_status == "blocked"
            else "ANALYSIS_DECISION_NOT_ACCEPTED"
        )
        return _preserve(
            previous,
            evidence,
    ...
    if (
        previous is not None
        and evidence.as_of == previous.as_of
        and evidence.delta_kind is not EvidenceDeltaKind.NO_OP
    ):
        return _preserve(
            previous,
            evidence,
    ...
    if analysis.market_stage_candidate != _expected_analysis_candidate(
        analysis.direction
    ):
        return _preserve(
            previous,
...
def ordinary_stage_distance(start: AnalysisStage, target: AnalysisStage) -> int:
    ...
    while frontier:
        ...
        frontier = {
            ...
            for adjacent in _STAGE_GRAPH[stage]
...
def _bootstrap(
    ...
    return _result(
        ...
        action=(
            ...
            else TransitionAction.STRENGTHEN
        ),
        transition_allowed=True,
...
def _ordinary(
    ...
    if _opposite(old_bias, new_bias):
        ...
        next_bias = (
            "mixed"
            if next_stage in {AnalysisStage.RANGE, AnalysisStage.DIRECTION_DECISION}
            else old_bias
        )
        return _advance(
            previous,
    ...
    if new_bias in {"bullish", "bearish"} and new_bias == old_bias:
        if (
            previous.pending_transition is not None
            and previous.pending_transition.rule is not PendingRule.TREND_ENTRY
        ):
            return _advance(
                previous,
                analysis,
...
def _conflict(
    ...
    next_bias = (
        "mixed"
        if next_stage in {AnalysisStage.DIRECTION_DECISION, AnalysisStage.RANGE}
        else previous.directional_bias
    )
    return _advance(
        previous,
        analysis,
...
def _major_confirmation(
    ...
    if (
        previous.stage is AnalysisStage.REVERSAL_WATCH
...
def _hard_invalidation(
    ...
    return _advance(
        ...
        action=TransitionAction.INVALIDATE,
        reason=(
            evidence.rule_code.value
            if evidence.rule_code is not None
            else "HARD_INVALIDATION"
        ),
    )
...
def _state(
    *,
    ...
    source_refs = _refs(
        ...
        *(
            ref
            for driver in (*analysis.dominant_drivers, *analysis.counter_drivers)
...
def _preserve_action(evidence: TransitionEvidence) -> TransitionAction:
    return (
        TransitionAction.MAINTAIN
        if evidence.delta_kind is EvidenceDeltaKind.NO_OP
        else TransitionAction.PENDING
    )
