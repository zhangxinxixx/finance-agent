...
def _v2_result(previous, state, evidence, reason, *, action=None) -> AnalysisStateTransitionV2Result:
    ...
    payload = {
        ...
        "action": action or _resolve_v2_transition_action(previous, state, evidence),
...
def _resolve_v2_transition_action(
    previous: AnalysisStateV2 | None,
    state: AnalysisStateV2,
    evidence: TransitionEvidence,
) -> TransitionAction:
    """Classify a v2 state change by business semantics, never enum fallback.

    Direction, regime, and maturity are orthogonal dimensions.  In particular,
    a completed bullish/bearish reversal is a direction switch, not evidence
    that strengthens the previous thesis.
    """

    if previous is None:
        # Preserve the bootstrap contract; PR-1 only adds an explicit action
        # where an existing thesis actually changes direction.
        return TransitionAction.STRENGTHEN
    if evidence.delta_kind is EvidenceDeltaKind.HARD_INVALIDATION:
        return TransitionAction.INVALIDATE
    if state.pending_transition is not None:
        return TransitionAction.PENDING

    direction_changed = previous.direction != state.direction or previous.direction_tilt != state.direction_tilt
    if direction_changed:
        return TransitionAction.SWITCH

    if (state.market_regime == "repair" and previous.market_regime != "repair") or (
        previous.market_regime == "trend" and state.market_regime != "trend"
    ):
        return TransitionAction.WEAKEN

    maturity_rank = {"invalidated": -1, "forming": 0, "watching": 1, "confirmed": 2}
    if maturity_rank[state.trend_maturity] < maturity_rank[previous.trend_maturity]:
        return TransitionAction.WEAKEN

    regime_rank = {"repair": -1, "pressure": 0, "range": 0, "direction_decision": 0, "trend": 1}
    if (
        regime_rank[state.market_regime] > regime_rank[previous.market_regime]
        or maturity_rank[state.trend_maturity] > maturity_rank[previous.trend_maturity]
    ):
        return TransitionAction.STRENGTHEN

    raise ValueError("v2 state change has no explicit transition action")
