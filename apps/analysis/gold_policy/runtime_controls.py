...
def build_gold_daily_close_runtime_controls(
    *,
    ...
    previous_transition: (
        StateTransitionPolicyDecision | StateTransitionPolicyDecisionV2 | None
    ) = None,
    ...
    options = _options_regime(current_feature, decision_time)
    ...
    if events.quality_status == "blocked" or events.freshness_status == "missing":
        ...
        active_ids = tuple(
            sorted(
                ...
                if event.occurred_at <= decision_as_of
...
def _refs(as_of: datetime, *refs: SourceReference) -> tuple[SourceReference, ...]:
    unique = {
        (ref.source, ref.reference, ref.retrieved_at): ref
        for ref in refs
        if ref.retrieved_at <= as_of
    }
    if not unique:
