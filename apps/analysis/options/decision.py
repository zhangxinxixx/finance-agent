...
def build_options_decision(
    ...
    *,
    ...
    gamma = _gamma_summary(snapshot, prices.get("live_p0"))
    ...
    key_levels = _key_levels(snapshot, prices, gamma)
    ...
    intraday = _intraday_strategy(gamma, key_levels, prices.get("live_p0"))
    ...
    return {
        ...
        "data_quality": {
            ...
            "live_price_context": {
                "status": "available" if prices.get("live_p0") is not None else "unavailable",
...
def _key_levels(
    snapshot: Mapping[str, Any], prices: Mapping[str, Any], gamma: Mapping[str, Any]
) -> list[dict[str, Any]]:
    levels: list[dict[str, Any]] = []
    live_p0 = _number_or_none(prices.get("live_p0"))
...
def _intraday_strategy(gamma: Mapping[str, Any], levels: list[dict[str, Any]], live_p0: float | None) -> dict[str, Any]:
    ...
    return {
        ...
        "horizon": "intraday",
        "regime": gamma["regime"],
        "bias": "defensive_repair" if downside else "mean_reversion",
        "summary": (
