...
REUTERS_PUBLIC_DISCOVERY_URL = "https://news.google.com/rss/search"
REUTERS_PUBLIC_SOURCE_NAME = "Reuters Public Metadata"
REUTERS_PUBLIC_QUERIES: dict[str, str] = {
    "middle_east_hormuz": 'site:reuters.com (Iran OR Israel) (Hormuz OR ceasefire OR "nuclear deal" OR sanctions)',
    "oil_supply": 'site:reuters.com (Brent OR WTI OR "oil supply" OR OPEC OR "shipping disruption")',
    "fed_inflation": 'site:reuters.com ("Federal Reserve" OR Fed OR FOMC) ("rate hike" OR "rate cut" OR inflation OR PCE OR CPI)',
    "gold_macro": 'site:reuters.com (gold OR XAU OR "COMEX gold") ("Treasury yields" OR dollar OR DXY OR "real yields")',
    "fx_yen": 'site:reuters.com (USDJPY OR "yen intervention" OR BOJ OR "Japan Ministry of Finance")',
}


def collect_reuters_public_news(
    *,
    retrieved_date: str,
    storage_root: Path,
    query_groups: dict[str, str] | None = None,
    max_items_per_group: int = 30,
    request_timeout: float | httpx.Timeout | None = 20.0,
    request_proxy: str | None = None,
    trust_env: bool = True,
    client: Any | None = None,
) -> NewsCollectionResult:
    queries = query_groups or REUTERS_PUBLIC_QUERIES
...
def _collect_single_query_group(
    *,
    ...
    try:
        response = client.get(REUTERS_PUBLIC_DISCOVERY_URL, params=params)
        ...
        warning = f"reuters_public_news:{query_group} {reason_code}: {reason}"
    ...
    raw_path = archive_news_payload(
        storage_root=storage_root,
        layer="raw",
        source_key="reuters_public",
        retrieved_date=retrieved_date,
        name=query_group,
        payload={
            "source_key": "reuters_public_news",
            "source_url": REUTERS_PUBLIC_DISCOVERY_URL,
            "query_group": query_group,
            "query": query,
            "params": params,
            "fetched_at": fetched_at,
            "discovery_method": "google_news_rss_site_filter",
            "xml": feed_text,
        },
    )
    try:
        parsed_items = _parse_reuters_public_items(
            feed_text=feed_text,
            query_group=query_group,
            query=query,
            fetched_at=fetched_at,
        ...
        unavailable_feeds.append(query_group)
        warning = f"reuters_public_news:{query_group} invalid_payload: {type(exc).__name__}: {exc}"
        source_refs.append(_build_source_ref(
            query_group=query_group,
            status="unavailable",
    ...
    warning = None
    if not parsed_items:
        warning = f"reuters_public_news:{query_group} no_public_candidates: authorized_wire=false public metadata only"
    source_refs.append(_build_source_ref(
        query_group=query_group,
        status="available" if parsed_items else "empty",
        reason=None if parsed_items else "No public Reuters candidates in Google News metadata; authorized wire access is not used",
        reason_code=None if parsed_items else "no_public_candidates",
        warning=warning,
        raw_path=raw_path,
...
def _parse_reuters_public_items(
    *,
    feed_text: str,
    query_group: str,
    query: str,
    ...
    for node in root.findall(".//item"):
        ...
        if not _is_reuters_public_candidate(url=url, publisher_url=publisher_url):
            continue
        duplicate_key = stable_news_item_id(source_key="reuters_public_news", title=title, url=url)
        result.append(RawNewsItem(
            source_key="reuters_public_news",
            source_name=REUTERS_PUBLIC_SOURCE_NAME,
            ...
            source_country="US",
            source_language="en",
            event_type=_classify_reuters_public_candidate(query_group=query_group, title=title),
...
def _is_reuters_public_candidate(*, url: str, publisher_url: str) -> bool:
    candidates = [url, publisher_url]
    return any(urlparse(candidate).netloc.lower().removeprefix("www.").endswith("reuters.com") for candidate in candidates if candidate)
...
def _classify_reuters_public_candidate(*, query_group: str, title: str) -> str:
...
def _build_source_ref(
    *,
    ...
    ref = {
        "source_ref": f"reuters_public_news:{query_group}",
        "source": "reuters_public_news",
        "query_group": query_group,
        "source_url": REUTERS_PUBLIC_DISCOVERY_URL,
        "status": status,
        "authorized_wire": False,
        "candidate_scope": "public_metadata_only",
        "discovery_method": "google_news_rss_site_filter",
    }
    if reason:
        ref["reason"] = reason
...
def _classify_request_exception(exc: Exception) -> tuple[str, str, str]:
    if isinstance(exc, httpx.HTTPStatusError):
        ...
        if status_code == 429:
            return ("rate_limited", "rate_limited", f"HTTP 429 from {REUTERS_PUBLIC_DISCOVERY_URL}")
        return ("unavailable", f"http_{status_code}", f"HTTP {status_code} from {REUTERS_PUBLIC_DISCOVERY_URL}")
